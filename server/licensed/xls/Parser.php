...
function new_ereg($patt,$str) {
  return preg_match("/$patt/",$str);
}
function new_eregi($patt,$str) {
  return preg_match("/$patt/i",$str);
}
function new_split($patt,$str) {
  return explode($patt,$str);
}
...
*/
  function _convert_range2d($range)
    ...
    if(preg_match("/^([A-I]?[A-Z])(\d+)\:([A-I]?[A-Z])(\d+)$/",$range)) {
        list($cell1, $cell2) = new_split(':', $range);
        }
...
*/
  function _match($token)
    ...
    switch($token)
        ...
        default:
	    // if it's a reference
            if(new_eregi("^[A-I]?[A-Z][0-9]+$",$token) and
	       !new_ereg("[0-9]",$this->_lookahead) and
               ($this->_lookahead != ':') and ($this->_lookahead != '.'))
                {
                return($token);
                }
            // if it's a range (A1:A2)
            elseif(new_eregi("^[A-I]?[A-Z][0-9]+:[A-I]?[A-Z][0-9]+$",$token) and
	           !new_ereg("[0-9]",$this->_lookahead))
	        {
		return($token);
		}
            // if it's a range (A1..A2)
		...
		return($token);
            ...
            elseif(new_eregi("^[A-Z0-9À-Ü\.]+$",$token) and ($this->_lookahead == "("))
...
*/
  function _fact()
    ...
    // if it's a reference
    if (new_eregi("^[A-I]?[A-Z][0-9]+$",$this->_current_token))
        {
    ...
    // if it's a range
    elseif (new_eregi("^[A-I]?[A-Z][0-9]+:[A-I]?[A-Z][0-9]+$",$this->_current_token) or
            new_eregi("^[A-I]?[A-Z][0-9]+\.\.[A-I]?[A-Z][0-9]+$",$this->_current_token))
        {
    ...
    // if it's a function call
    elseif (new_eregi("^[A-Z0-9À-Ü\.]+$",$this->_current_token))
        {
        $result = $this->_func();
        return($result);
        }
