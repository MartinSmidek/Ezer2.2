...
class Worksheet extends BIFFwriter
{
    ...
    */
    function _write_url_external($row1, $col1, $row2, $col2, $url, $str, $format = 0)
    {
        ...
        list($dir_long , $sheet) = new_split('#', $url);  //split('/\#/', $url);
        $link_type               = 0x01 | $absolute;
    
        if (isset($sheet)) {
            $link_type |= 0x08;
            $sheet_len  = pack("V", strlen($sheet) + 0x01);
            $sheet      = join("\0", new_split('', $sheet));
            $sheet     .= "\0\0\0";
        }
        else {
            $sheet_len   = '';
        ...
        $dir_long       = join("\0", new_split('', $dir_long));
        $dir_long       = $dir_long . "\0";
    ...
    sub _write_url_external_net {
        ...
        # Split the dir name and sheet name (if it exists)
        #
        my ($dir_long , $sheet) = new_split('#', $url);  //split('/\#/', $url);
