...
//t: Block
//s: Block
Ezer.Report= new Class({
  Extends: Ezer.Block,
  options: {},
  status: 'start',      // 'start', 'init', 'check'
  ctx: {},              // kontext pro výpočet rozměrů
  zoom: 100,            // zoom v procentech zvětšení (100 odpovídá 1:1)
  outline: 0,           // 0 nic, 1 pojmenované boxy, 2 všechny boxy
  pages: [],            // pole výšek jednotlivých stránek reportu
  type: '',             // 'batch' ovlivní funkci report_check a report_print
  batch: '',            // zpracovaná dávka (pokud type=='batch') pro print
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -  initialize
...
//   pretty - 1 odsaď vygenerované html
  get_html: function (zoom,outline,pretty) {
    if ( this.status=='check' || this.status=='pages' ) {
      this.outline= outline||0;
      this.zoom= 100; //zoom||100;
      var c= this.options, style= AE_style_bob(c._l,c._t,c._w,c._h,this.zoom);
      var html= "<div class='report' style='"+style+"'>\n";
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
function AE_style_bob (l,t,w,h,zoom) {
  l= l*zoom/100; t= t*zoom/100; w= w*zoom/100; h= h*zoom/100;
  var s= 'left:'+l+'mm;top:'+t+'mm;width:'+w+'mm;height:'+h+'mm;';
  return s;
}
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
function AE_copy_bob (bob,up,sb) {
  var copy= new Ezer.Bob(bob.box,up,sb);
  ...
  for (var ib= 0; ib<bob.tree.length; ib++) {
    sb= copy.tree[ib]= AE_copy_bob(bob.tree[ib],copy,sb);
  }
  return copy;
}
// ================================================================================================= Bob
...
Ezer.Bob= new Class({
  box: null,            // statický vzor boxu - QE_box
  up: null,             // otec
  sb: null,             // starší bratr (uzel vlevo||null)
  value: '',            // hodnota
  tree: [],             // strom podboxů vytvořených funkcí graft
  pagebreak: false,     // true vyvolá přechod na novou stránku pro tento bob
  // spočtené polohové atributy
  l: -1, t: -1, r: -1, b: -1, w: -1, h: -1,
  // spočtený pravý dolní roh obsahujícího potenciálního boxu
  max: {r:0,b:0},
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -  initialize
  initialize: function(box,up,sb) {
    this.box= box;
    this.up= up;
    this.sb= sb;
  },
...
// a = L|t|w|h ... pro l|t se velikost počítá relativně k nadřazenému bobu
  AE_check: function (bob,a,ctx,sum) {
  ...
  //                 back_refs++;
      ...
      case '*': var trep= $('report'), wpx, hpx;
//                 var tbob= AE_appendChild(trep, { tag:'div', className:bob.box.css, innerHTML:bob.value });
                var tbob= new Element('div', {'class':bob.box.css, html:bob.value }).inject(trep);
                if ( a=='h' ) tbob.style.width= bob.w+'mm'; else tbob.style.height= bob.h+'mm';
                var wpx= tbob.offsetWidth;
                var hpx= tbob.offsetHeight;
...
// provede vložení přechodů na novou stránku a vrátí maximální výšku té staré
  check_page: function (tree,it,nb) {
    var end= 0;
        ...
        if ( tree && (id= apb[1]) ) {
          ...
          for (var sb= this.sb; sb; sb= sb.sb) {
            if ( id==sb.box.id ) {
              ...
              bob= AE_copy_bob(sb,this.up,this.sb);
              this.sb= bob;
              bob.pagebreak= true;
...
//      vrátí HTML řetězec dynamického boxu, !height je vnucená výška vnořených elementů
  get_html: function (pretty,forced_h) {
    var zoom= this.box.report.zoom;
    var style= AE_style_bob(this.l,this.t,this.w,forced_h?forced_h:this.h,zoom);
...
//      !height je vnucená výška vnořených elementů
  get_page: function (t1,t2,forced_h) {
    var html= '', b= this.t+this.h;
    if ( t1 <= this.t && b <= t2 ) {
      var style= AE_style_bob(this.l,this.t-t1,this.w,forced_h?forced_h:this.h,100);
      var css= this.box.css;
      var title= '';
...
Ezer.Box= new Class({
  Extends: Ezer.Block,
  options: {},
  value: '',         // nastavuje se attr.title nebo let
  css: '',           // nastavuje se jako box_<attr.css|text>
  style: '',         // obsahuje coord jako html atribut
  open: null,        // otevřená větev pro roubování - QE_bob
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -  initialize
  initialize: function(owner,desc,DOM,id,skill) {
    this.parent(owner,desc,DOM,id,skill);
    // report boxu
